...
reorder_columns <- function(df, ces_vars) {
  for (ces in ces_vars) {
    ces_columns <- paste0("source_id.", ces, c(".cleaned", ".question_label"))
...
pivot_and_add_year <- function(df, ces_var) {
  ces_columns <- paste0("source_id.", ces_var, c(".cleaned", ".question_label"))
  df_long <- df %>%
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("JSON", "Sélectionner le JSON:",choices = unique(df_long_combined$value)),
